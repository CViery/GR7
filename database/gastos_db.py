...
class GastosDataBase:
    ...
    def get_tabela_boleto(self):
        try:
            if session['empresa'] == 1:
                return "boletos"
            elif session['empresa'] == 2:
                return "boletos_portal"
            elif session['empresa'] == 3:
                return "boletos_morumbi"
        except Exception as e:
            # Loga o erro detalhado
            logging.error("Erro ao cadastrar faturamento: %s", e)
    ...
    def set_boleto(self, boleto):
        try:
            ...
            tabela = self.get_tabela_boleto()
            # Preparar a query de inserção
            query = f'INSERT INTO {tabela} (num_nota, notas, fornecedor, data_vencimento, dia_vencimento, mes_vencimento, ano_vencimento, valor) VALUES (?,?,?,?,?,?,?,?)'
    ...
    def get_all_gastos(self):
        ...
        try:
            ...
            tabela = self.get_tabela_notas()
            # Definindo a query
            query = 'SELECT * FROM {nota} ORDER BY data_emissao ASC;'
